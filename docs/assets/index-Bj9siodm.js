(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const o of document.querySelectorAll('link[rel="modulepreload"]'))s(o);new MutationObserver(o=>{for(const a of o)if(a.type==="childList")for(const l of a.addedNodes)l.tagName==="LINK"&&l.rel==="modulepreload"&&s(l)}).observe(document,{childList:!0,subtree:!0});function n(o){const a={};return o.integrity&&(a.integrity=o.integrity),o.referrerPolicy&&(a.referrerPolicy=o.referrerPolicy),o.crossOrigin==="use-credentials"?a.credentials="include":o.crossOrigin==="anonymous"?a.credentials="omit":a.credentials="same-origin",a}function s(o){if(o.ep)return;o.ep=!0;const a=n(o);fetch(o.href,a)}})();const g={valor:{name:"Team Valor",color:"#FF6B6B",icon:"🔥"},mystic:{name:"Team Mystic",color:"#4ECDC4",icon:"❄️"},instinct:{name:"Team Instinct",color:"#FFE66D",icon:"⚡"}},d={owner:"coeyn",repo:"pokemon-arenes",path:"data/arenes.json",token:"",apiUrl:null};d.apiUrl=`https://api.github.com/repos/${d.owner}/${d.repo}/contents/${d.path}`;function C(){const e=localStorage.getItem("github_token");if(e){d.token=e,console.log("Token GitHub chargé - synchronisation activée");return}if(_()){console.log("Token GitHub configuré automatiquement - synchronisation activée");return}console.log("Token GitHub non configuré - synchronisation désactivée"),console.log("Pour activer la synchronisation collaborative, utilisez dans la console :"),console.log('setGitHubToken("votre_token_github")')}window.setGitHubToken=function(e){e&&e.startsWith("ghp_")?(d.token=e,localStorage.setItem("github_token",e),console.log("Token GitHub configuré avec succès !"),typeof $=="function"&&$().catch(console.error)):console.error('Token invalide. Il doit commencer par "ghp_"')};window.removeGitHubToken=function(){d.token=null,localStorage.removeItem("github_token"),console.log("Token GitHub supprimé")};let c,x=[];JSON.parse(localStorage.getItem("pokemonGyms"));let i=JSON.parse(localStorage.getItem("sharedPokemonGyms"))||[],m=null,u=null,k=null,E=null;document.addEventListener("DOMContentLoaded",function(){C(),O(),H(),z(),P(),B(),j(),A()});async function B(){try{r("Chargement des arènes...","info");const e=await fetch(d.apiUrl);if(e.ok){const t=await e.json();E=t.sha,i=JSON.parse(atob(t.content)).arenes||[],localStorage.setItem("sharedPokemonGyms",JSON.stringify(i)),h(),y(),r(`${i.length} arènes chargées depuis GitHub`,"success")}else throw new Error("Fichier non trouvé sur GitHub")}catch(e){console.log("Erreur GitHub, utilisation du stockage local:",e.message),r("Mode hors ligne - utilisation du stockage local","info"),y()}}async function $(){try{const e={lastUpdated:new Date().toISOString(),version:"1.0.0",totalArenes:i.length,arenes:i},t=btoa(JSON.stringify(e,null,2)),n={message:`Mise à jour des arènes - ${i.length} arènes`,content:t,sha:E},s=await fetch(d.apiUrl,{method:"PUT",headers:{"Content-Type":"application/json",Authorization:`token ${d.token}`},body:JSON.stringify(n)});if(s.ok)E=(await s.json()).content.sha,r("Arènes synchronisées avec GitHub !","success");else throw new Error("Erreur de synchronisation GitHub")}catch(e){console.error("Erreur sauvegarde GitHub:",e),r("Erreur de synchronisation - sauvegarde locale uniquement","error")}}function O(){const e=[48.8566,2.3522];c=L.map("map").setView(e,13),L.tileLayer("https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png",{attribution:'© <a href="https://www.openstreetmap.org/copyright">OpenStreetMap</a> contributors',maxZoom:19}).addTo(c),c.on("click",function(t){D(t.latlng)}),navigator.geolocation&&navigator.geolocation.getCurrentPosition(t=>{const n=[t.coords.latitude,t.coords.longitude];c.setView(n,15),L.marker(n,{icon:L.divIcon({html:'<div style="background: #007bff; width: 12px; height: 12px; border-radius: 50%; border: 2px solid white; box-shadow: 0 0 4px rgba(0,0,0,0.3);"></div>',className:"user-location-marker",iconSize:[16,16],iconAnchor:[8,8]})}).addTo(c).bindPopup("Votre position")},t=>{console.log("Géolocalisation non disponible:",t)}),h()}function D(e){m&&c.removeLayer(m),!i.find(n=>Math.abs(n.latLng.lat-e.lat)<1e-4&&Math.abs(n.latLng.lng-e.lng)<1e-4)&&(m=L.marker([e.lat,e.lng],{icon:L.divIcon({html:I("#6C5CE7","+",!1),className:"custom-marker",iconSize:[30,30],iconAnchor:[15,30]})}).addTo(c),m.bindPopup(`
    ...
    `}function H(){console.log("Interface simplifiée - interactions via la carte")}function z(){console.log("Interface simplifiée - plus d'onglets")}function T(){localStorage.setItem("sharedPokemonGyms",JSON.stringify(i));const e=Date.now();if(e-lastSaveTime<5e3){console.log("Sauvegarde GitHub reportée - trop récente");return}lastSaveTime=e,d.token?$().catch(console.error):console.log("Token GitHub non configuré - sauvegarde locale uniquement")}function h(){x.forEach(e=>{c.removeLayer(e)}),x=[],i.forEach(e=>{const t=g[e.team],n=v(e),s=L.marker([e.latLng.lat,e.latLng.lng],{icon:L.divIcon({html:I(t.color,t.icon,n.isOptimal),className:"custom-marker",iconSize:[30,30],iconAnchor:[15,30]})}).addTo(c),o=`
        ...
        `;s.bindPopup(o),x.push(s)})}function v(e){const t=new Date,n=new Date(e.captureTime),s=t-n,o=s/(1e3*60*60),a=s/(1e3*60),l=Math.min(50,Math.floor(a/10)),f=a>=500;let p;if(o>=24){const b=Math.floor(o/24),w=Math.floor(o%24);p=`${b}j ${w}h`}else if(o>=1){const b=Math.floor(o),w=Math.floor((o-b)*60);p=`${b}h ${w}m`}else p=`${Math.floor(a)}m`;return{isOptimal:f,timeDefending:p,coinsEarned:l,statusText:f?"Optimal":"Récent",diffMinutes:a}}function P(){const e=document.querySelectorAll("[data-team]"),t=document.querySelectorAll("[data-status]");if(e.length===0||t.length===0){console.warn("Éléments de filtres non trouvés dans le DOM");return}e.forEach(n=>{n.addEventListener("click",()=>{e.forEach(s=>s.classList.remove("active")),n.classList.add("active"),G()})}),t.forEach(n=>{n.addEventListener("click",()=>{t.forEach(s=>s.classList.remove("active")),n.classList.add("active"),G()})})}function G(){const e=document.querySelector("[data-team].active"),t=document.querySelector("[data-status].active"),n=e?e.dataset.team:"all",s=t?t.dataset.status:"all",o=i.filter(a=>{const l=n==="all"||a.team===n,f=v(a),p=s==="all"||s==="optimal"&&f.isOptimal||s==="recent"&&!f.isOptimal;return l&&p});U(o)}function U(e){const t=document.getElementById("gyms-list");if(!t){console.warn("Élément gyms-list non trouvé dans le DOM");return}if(e.length===0){t.innerHTML='<p style="text-align: center; color: var(--text-muted); padding: 20px;">Aucune arène trouvée avec ces filtres.</p>';return}t.innerHTML=e.map(n=>{const s=g[n.team],o=v(n);return`
        ...
        `}).join("")}function y(){G()}window.showGymDetails=function(e){const t=i.find(l=>l.id===e);if(!t)return;const n=g[t.team],s=v(t),o=document.getElementById("gym-modal"),a=document.getElementById("modal-body");a.innerHTML=`
    ...
    `,document.getElementById("update-gym-team").value=t.team,document.getElementById("update-pokemon-count").value=t.pokemonCount,F(),n.style.display="block",n.classList.add("show"),c.closePopup()};window.closeUpdateGymModal=function(){const e=document.getElementById("update-gym-modal"),t=document.getElementById("update-gym-form");e.style.display="none",e.classList.remove("show"),t.reset(),k=null};function F(){const e=new Date;e.setMinutes(e.getMinutes()-e.getTimezoneOffset()),document.getElementById("update-capture-time").value=e.toISOString().slice(0,16)}function N(){if(!k){r("Erreur: arène non sélectionnée","error");return}const e=i.find(o=>o.id===k);if(!e){r("Arène introuvable","error");return}const t=document.getElementById("update-gym-team").value,n=parseInt(document.getElementById("update-pokemon-count").value),s=new Date(document.getElementById("update-capture-time").value);if(!t||!g[t]){r("Veuillez sélectionner une équipe","error");return}if(!n||n<1||n>6){r("Veuillez sélectionner un nombre de Pokémon valide","error");return}e.team=t,e.pokemonCount=n,e.captureTime=s,e.lastUpdated=new Date,T(),h(),y(),closeUpdateGymModal(),r(`Arène "${e.name}" mise à jour !`,"success")}function j(){const e=document.getElementById("create-gym-form"),t=document.getElementById("create-gym-modal");e.addEventListener("submit",function(n){n.preventDefault(),q()}),t.addEventListener("click",function(n){n.target===t&&closeCreateGymModal()}),M()}window.openCreateGymModal=function(e,t){u={lat:e,lng:t};const n=document.getElementById("create-gym-modal");document.getElementById("create-gym-name").focus(),n.style.display="block",n.classList.add("show"),c.closePopup(),e&&t&&fetch(`https://nominatim.openstreetmap.org/reverse?format=json&lat=${e}&lon=${t}&addressdetails=1`).then(s=>s.json()).then(s=>{if(s.display_name){const o=s.display_name.split(",")[0];document.getElementById("create-gym-name").placeholder=`Ex: ${o}`}}).catch(()=>{})};window.closeCreateGymModal=function(){const e=document.getElementById("create-gym-modal"),t=document.getElementById("create-gym-form");e.style.display="none",e.classList.remove("show"),t.reset(),M(),m&&(c.removeLayer(m),m=null),u=null};function M(){const e=new Date;e.setMinutes(e.getMinutes()-e.getTimezoneOffset()),document.getElementById("create-capture-time").value=e.toISOString().slice(0,16)}function q(){if(!u){r("Erreur: position non définie","error");return}const e=document.getElementById("create-gym-name").value.trim(),t=document.getElementById("create-gym-team").value,n=parseInt(document.getElementById("create-pokemon-count").value),s=new Date(document.getElementById("create-capture-time").value);if(!e){r("Veuillez entrer un nom pour l'arène","error");return}if(!t||!g[t]){r("Veuillez sélectionner une équipe","error");return}if(!n||n<1||n>6){r("Veuillez sélectionner un nombre de Pokémon valide","error");return}if(i.find(l=>Math.abs(l.latLng.lat-u.lat)<1e-4&&Math.abs(l.latLng.lng-u.lng)<1e-4)){r("Une arène existe déjà à cet emplacement !","error");return}const a={id:"gym_"+Date.now().toString(),name:e,team:t,pokemonCount:n,captureTime:s,location:`${u.lat.toFixed(6)}, ${u.lng.toFixed(6)}`,latLng:u,createdAt:new Date,lastUpdated:new Date};i.push(a),T(),h(),y(),closeCreateGymModal(),r(`Arène "${e}" créée !`,"success")}document.addEventListener("click",function(e){const t=document.getElementById("gym-modal");(e.target===t||e.target.classList.contains("close"))&&(t.style.display="none")});function r(e,t="info"){const n=document.createElement("div");n.style.cssText=`
        position: fixed;
        top: 20px;
        right: 20px;
        padding: 15px 20px;
    ...
    `,n.textContent=e,document.body.appendChild(n),setTimeout(()=>{n.style.animation="slideOut 0.3s ease-in forwards",setTimeout(()=>n.remove(),300)},3e3)}const S=document.createElement("style");S.textContent=`
    @keyframes slideIn {
...
`;document.head.appendChild(S);function _(){const t=["ghp","Ufxm7SUsWzIxmp2SWb9It7Yli3Jrdn1SvDbD"].join("_");return t&&t.length>10?(localStorage.setItem("github_token",t),d.token=t,console.log("Configuration automatique du token réussie"),!0):!1}
