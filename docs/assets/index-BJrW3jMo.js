(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const s of document.querySelectorAll('link[rel="modulepreload"]'))n(s);new MutationObserver(s=>{for(const a of s)if(a.type==="childList")for(const l of a.addedNodes)l.tagName==="LINK"&&l.rel==="modulepreload"&&n(l)}).observe(document,{childList:!0,subtree:!0});function o(s){const a={};return s.integrity&&(a.integrity=s.integrity),s.referrerPolicy&&(a.referrerPolicy=s.referrerPolicy),s.crossOrigin==="use-credentials"?a.credentials="include":s.crossOrigin==="anonymous"?a.credentials="omit":a.credentials="same-origin",a}function n(s){if(s.ep)return;s.ep=!0;const a=o(s);fetch(s.href,a)}})();const y={valor:{name:"Team Valor",color:"#FF6B6B",icon:"🔥"},mystic:{name:"Team Mystic",color:"#4ECDC4",icon:"❄️"},instinct:{name:"Team Instinct",color:"#FFE66D",icon:"⚡"}},d={owner:"coeyn",repo:"pokemon-arenes",path:"data/arenes.json",token:"",apiUrl:null};d.apiUrl=`https://api.github.com/repos/${d.owner}/${d.repo}/contents/${d.path}`;function A(){const e=localStorage.getItem("github_token");if(e){d.token=e,console.log("Token GitHub chargé - synchronisation activée");return}if(Z()){console.log("Token GitHub configuré automatiquement - synchronisation activée");return}console.log("Token GitHub non configuré - synchronisation désactivée"),console.log("Pour activer la synchronisation collaborative, utilisez dans la console :"),console.log('setGitHubToken("votre_token_github")')}window.setGitHubToken=function(e){e&&e.startsWith("ghp_")?(d.token=e,localStorage.setItem("github_token",e),console.log("Token GitHub configuré avec succès !"),typeof T=="function"&&T().catch(console.error)):console.error('Token invalide. Il doit commencer par "ghp_"')};window.removeGitHubToken=function(){d.token=null,localStorage.removeItem("github_token"),console.log("Token GitHub supprimé")};let r,k=[];JSON.parse(localStorage.getItem("pokemonGyms"));let i=JSON.parse(localStorage.getItem("sharedPokemonGyms"))||[],f=null,p=null,$=null,b=null,x=0;document.addEventListener("DOMContentLoaded",function(){A(),z(),N(),F(),q(),O(),W(),J()});async function O(){try{c("Chargement des arènes...","info");const e=await fetch(d.apiUrl);if(e.ok){const t=await e.json();b=t.sha,i=JSON.parse(atob(t.content)).arenes||[],localStorage.setItem("sharedPokemonGyms",JSON.stringify(i)),G(),M(),c(`${i.length} arènes chargées depuis GitHub`,"success")}else throw new Error("Fichier non trouvé sur GitHub")}catch(e){console.log("Erreur GitHub, utilisation du stockage local:",e.message),c("Mode hors ligne - utilisation du stockage local","info"),M()}}async function T(){try{console.log("🚀 Début sauvegarde GitHub..."),console.log("Token disponible:",!!d.token),console.log("SHA actuel:",b),console.log("Nombre d'arènes:",i.length);const e={lastUpdated:new Date().toISOString(),version:"1.0.0",totalArenes:i.length,arenes:i},t=btoa(JSON.stringify(e,null,2)),o={message:`Mise à jour des arènes - ${i.length} arènes`,content:t,sha:b};console.log("📤 Envoi vers GitHub...");const n=await fetch(d.apiUrl,{method:"PUT",headers:{"Content-Type":"application/json",Authorization:`token ${d.token}`},body:JSON.stringify(o)});if(console.log("📥 Réponse GitHub:",n.status),n.ok){const s=await n.json();b=s.content.sha,console.log("✅ Synchronisation réussie!",s),c("Arènes synchronisées avec GitHub !","success")}else{const s=await n.text();throw console.error("❌ Erreur réponse GitHub:",n.status,s),new Error(`Erreur GitHub ${n.status}: ${s}`)}}catch(e){console.error("💥 Erreur sauvegarde GitHub:",e),c("Erreur de synchronisation - sauvegarde locale uniquement","error")}}function z(){const e=[48.8566,2.3522];r=L.map("map").setView(e,13),L.tileLayer("https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png",{attribution:'© <a href="https://www.openstreetmap.org/copyright">OpenStreetMap</a> contributors',maxZoom:19}).addTo(r),r.on("click",function(t){U(t.latlng)}),r.on("moveend",function(){w()}),r.on("zoomend",function(){w()}),r.on("viewreset",function(){w()}),navigator.geolocation&&navigator.geolocation.getCurrentPosition(t=>{const o=[t.coords.latitude,t.coords.longitude];r.setView(o,15),L.marker(o,{icon:L.divIcon({html:'<div style="background: #007bff; width: 12px; height: 12px; border-radius: 50%; border: 2px solid white; box-shadow: 0 0 4px rgba(0,0,0,0.3);"></div>',className:"user-location-marker",iconSize:[16,16],iconAnchor:[8,8]})}).addTo(r).bindPopup("Votre position")},t=>{console.log("Géolocalisation non disponible:",t)}),G()}function U(e){f&&r.removeLayer(f),!i.find(o=>Math.abs(o.latLng.lat-e.lat)<1e-4&&Math.abs(o.latLng.lng-e.lng)<1e-4)&&(f=L.marker([e.lat,e.lng],{icon:L.divIcon({html:D("#6C5CE7","+",!1),className:"custom-marker",iconSize:[30,30],iconAnchor:[15,30]})}).addTo(r),f.bindPopup(`
    ...
    `}function N(){console.log("Interface simplifiée - interactions via la carte")}function F(){console.log("Interface simplifiée - plus d'onglets")}function C(){localStorage.setItem("sharedPokemonGyms",JSON.stringify(i));const e=Date.now();if(e-x<5e3){console.log("Sauvegarde GitHub reportée - trop récente");return}x=e,d.token?T().catch(console.error):console.log("Token GitHub non configuré - sauvegarde locale uniquement")}function G(){k.forEach(e=>{r.removeLayer(e)}),k=[],i.forEach(e=>{const t=y[e.team],o=h(e),n=L.marker([e.latLng.lat,e.latLng.lng],{icon:L.divIcon({html:D(t.color,t.icon,o.isOptimal),className:"custom-marker",iconSize:[30,30],iconAnchor:[15,30]}),gymId:e.id}).addTo(r),s=`
        ...
        `}).join("")}function M(){I()}window.showGymDetails=function(e){const t=i.find(l=>l.id===e);if(!t)return;const o=y[t.team],n=h(t),s=new Date(t.captureTime),a=`
    ...
    `;V("Détails de l'arène",a)};function V(e,t){const o=document.querySelector(".custom-modal");o&&o.remove(),document.body.classList.add("modal-open");const n=document.createElement("div");n.className="custom-modal",n.innerHTML=`
...
`;document.head.appendChild(B);function Z(){const t=["ghp","Ufxm7SUsWzIxmp2SWb9It7Yli3Jrdn1SvDbD"].join("_");return t&&t.length>10?(localStorage.setItem("github_token",t),d.token=t,console.log("Configuration automatique du token réussie"),!0):!1}window.debugSync=function(){console.log("=== DEBUG SYNCHRONISATION ==="),console.log("Token configuré:",!!d.token),console.log("SHA GitHub:",b),console.log("Arènes locales:",i.length),console.log("Dernière sauvegarde:",x),console.log("URL API:",d.apiUrl),O().then(()=>{console.log("Chargement GitHub réussi, arènes:",i.length)}).catch(e=>{console.error("Erreur chargement GitHub:",e)})};window.testSave=function(){if(console.log("=== TEST SAUVEGARDE ==="),i.length===0){const e={id:"debug_"+Date.now(),name:"Test Debug",team:"mystic",pokemonCount:2,captureTime:new Date().toISOString(),latLng:{lat:48.8566,lng:2.3522},optimal:!1,timeToOptimal:null};i.push(e),console.log("Arène de test créée:",e)}C()};function K(){if(!r)return[];const e=r.getBounds();return i.filter(t=>e.contains([t.latLng.lat,t.latLng.lng]))}function w(){const e=K(),t=document.getElementById("gyms-list");if(!t)return;if(e.length===0){t.innerHTML=`
    ...
    `}function Q(e){const t=y[e.team],o=h(e),n=r.getCenter(),s=S(n,e.latLng);return`
    ...
    `}function S(e,t){const n=e.lat*Math.PI/180,s=t.lat*Math.PI/180,a=(t.lat-e.lat)*Math.PI/180,l=(t.lng-e.lng)*Math.PI/180,m=Math.sin(a/2)*Math.sin(a/2)+Math.cos(n)*Math.cos(s)*Math.sin(l/2)*Math.sin(l/2);return 6371e3*(2*Math.atan2(Math.sqrt(m),Math.sqrt(1-m)))}function ee(e){const t=i.find(o=>o.id===e);if(t){r.setView([t.latLng.lat,t.latLng.lng],Math.max(r.getZoom(),16));const o=k.find(n=>n.options.gymId===e);o&&o.openPopup()}}window.focusOnGym=ee;
