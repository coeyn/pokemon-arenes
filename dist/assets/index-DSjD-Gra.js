(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const o of document.querySelectorAll('link[rel="modulepreload"]'))s(o);new MutationObserver(o=>{for(const a of o)if(a.type==="childList")for(const l of a.addedNodes)l.tagName==="LINK"&&l.rel==="modulepreload"&&s(l)}).observe(document,{childList:!0,subtree:!0});function n(o){const a={};return o.integrity&&(a.integrity=o.integrity),o.referrerPolicy&&(a.referrerPolicy=o.referrerPolicy),o.crossOrigin==="use-credentials"?a.credentials="include":o.crossOrigin==="anonymous"?a.credentials="omit":a.credentials="same-origin",a}function s(o){if(o.ep)return;o.ep=!0;const a=n(o);fetch(o.href,a)}})();const g={valor:{name:"Team Valor",color:"#FF6B6B",icon:"🔥"},mystic:{name:"Team Mystic",color:"#4ECDC4",icon:"❄️"},instinct:{name:"Team Instinct",color:"#FFE66D",icon:"⚡"}},p={owner:"coeyn",repo:"pokemon-arenes",path:"data/arenes.json",apiUrl:null};p.apiUrl=`https://api.github.com/repos/${p.owner}/${p.repo}/contents/${p.path}`;let c,$=[];JSON.parse(localStorage.getItem("pokemonGyms"));let i=JSON.parse(localStorage.getItem("sharedPokemonGyms"))||[],m=null,d=null,x=null,k=null;document.addEventListener("DOMContentLoaded",function(){S(),D(),z(),P(),T(),H(),A()});async function T(){try{r("Chargement des arènes...","info");const e=await fetch(p.apiUrl);if(e.ok){const t=await e.json();k=t.sha,i=JSON.parse(atob(t.content)).arenes||[],h(),v(),y(),r(`${i.length} arènes chargées depuis GitHub`,"success")}else throw new Error("Fichier non trouvé sur GitHub")}catch(e){console.log("Erreur GitHub, utilisation du stockage local:",e.message),r("Mode hors ligne - utilisation du stockage local","info"),y()}}async function B(){try{const e={lastUpdated:new Date().toISOString(),version:"1.0.0",totalArenes:i.length,arenes:i},t=btoa(JSON.stringify(e,null,2)),n={message:`Mise à jour des arènes - ${i.length} arènes`,content:t,sha:k},s=await fetch(p.apiUrl,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify(n)});if(s.ok)k=(await s.json()).content.sha,r("Arènes synchronisées avec GitHub !","success");else throw new Error("Erreur de synchronisation GitHub")}catch(e){console.error("Erreur sauvegarde GitHub:",e),r("Erreur de synchronisation - sauvegarde locale uniquement","error"),h()}}function S(){const e=[48.8566,2.3522];c=L.map("map").setView(e,13),L.tileLayer("https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png",{attribution:'© <a href="https://www.openstreetmap.org/copyright">OpenStreetMap</a> contributors',maxZoom:19}).addTo(c),c.on("click",function(t){O(t.latlng)}),navigator.geolocation&&navigator.geolocation.getCurrentPosition(t=>{const n=[t.coords.latitude,t.coords.longitude];c.setView(n,15),L.marker(n,{icon:L.divIcon({html:'<div style="background: #007bff; width: 12px; height: 12px; border-radius: 50%; border: 2px solid white; box-shadow: 0 0 4px rgba(0,0,0,0.3);"></div>',className:"user-location-marker",iconSize:[16,16],iconAnchor:[8,8]})}).addTo(c).bindPopup("Votre position")},t=>{console.log("Géolocalisation non disponible:",t)}),v()}function O(e){m&&c.removeLayer(m),!i.find(n=>Math.abs(n.latLng.lat-e.lat)<1e-4&&Math.abs(n.latLng.lng-e.lng)<1e-4)&&(m=L.marker([e.lat,e.lng],{icon:L.divIcon({html:G("#6C5CE7","+",!1),className:"custom-marker",iconSize:[30,30],iconAnchor:[15,30]})}).addTo(c),m.bindPopup(`
        ...
        `).openPopup())}function G(e,t,n){return`
        <div style="
            width: 30px; 
            height: 30px; 
            background: ${e}; 
    ...
    `}function D(){console.log("Interface simplifiée - interactions via la carte")}function z(){console.log("Interface simplifiée - plus d'onglets")}function h(){localStorage.setItem("sharedPokemonGyms",JSON.stringify(i)),B().catch(console.error)}function v(){$.forEach(e=>{c.removeLayer(e)}),$=[],i.forEach(e=>{const t=g[e.team],n=b(e),s=L.marker([e.latLng.lat,e.latLng.lng],{icon:L.divIcon({html:G(t.color,t.icon,n.isOptimal),className:"custom-marker",iconSize:[30,30],iconAnchor:[15,30]})}).addTo(c),o=`
        ...
        `;s.bindPopup(o),$.push(s)})}function b(e){const t=new Date,n=new Date(e.captureTime),s=t-n,o=s/(1e3*60*60),a=s/(1e3*60),l=Math.min(50,Math.floor(a/10)),f=a>=500;let u;if(o>=24){const w=Math.floor(o/24),E=Math.floor(o%24);u=`${w}j ${E}h`}else if(o>=1){const w=Math.floor(o),E=Math.floor((o-w)*60);u=`${w}h ${E}m`}else u=`${Math.floor(a)}m`;return{isOptimal:f,timeDefending:u,coinsEarned:l,statusText:f?"Optimal":"Récent",diffMinutes:a}}function P(){const e=document.querySelectorAll("[data-team]"),t=document.querySelectorAll("[data-status]");if(e.length===0||t.length===0){console.warn("Éléments de filtres non trouvés dans le DOM");return}e.forEach(n=>{n.addEventListener("click",()=>{e.forEach(s=>s.classList.remove("active")),n.classList.add("active"),M()})}),t.forEach(n=>{n.addEventListener("click",()=>{t.forEach(s=>s.classList.remove("active")),n.classList.add("active"),M()})})}function M(){const e=document.querySelector("[data-team].active"),t=document.querySelector("[data-status].active"),n=e?e.dataset.team:"all",s=t?t.dataset.status:"all",o=i.filter(a=>{const l=n==="all"||a.team===n,f=b(a),u=s==="all"||s==="optimal"&&f.isOptimal||s==="recent"&&!f.isOptimal;return l&&u});U(o)}function U(e){const t=document.getElementById("gyms-list");if(!t){console.warn("Élément gyms-list non trouvé dans le DOM");return}if(e.length===0){t.innerHTML='<p style="text-align: center; color: var(--text-muted); padding: 20px;">Aucune arène trouvée avec ces filtres.</p>';return}t.innerHTML=e.map(n=>{const s=g[n.team],o=b(n);return`
        ...
        `}).join("")}function y(){M()}window.showGymDetails=function(e){const t=i.find(l=>l.id===e);if(!t)return;const n=g[t.team],s=b(t),o=document.getElementById("gym-modal"),a=document.getElementById("modal-body");a.innerHTML=`
